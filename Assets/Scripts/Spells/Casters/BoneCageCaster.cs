...
public class BoneCageCaster : SpellCaster
{
    [SerializeField] private GameObject _indicatorPrefab;
    [SerializeField] private GameObject _boneCagePrefab;
    ...
    void Update()
    {
        ...
        if (_castingSpell && 
            _hand.Grabber.transform.position.y > _startYPos + _yPosThreshold) 
        {
            _castingSpell = false;

            Instantiate(_boneCagePrefab, _indicator.transform.position, Quaternion.Euler(-90, 0, 0));
            Destroy(_indicator);
        }  
    }
}
